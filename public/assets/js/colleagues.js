$(document).ready(() => {
    ...
    const employeeDataUrl = $('meta[name="colleagues-data-url"]').attr('content');

    // Initialize DataTable with server-side processing
    const initializeDataTable = () => {
        return $('#kt_colleagues_table').DataTable({
            processing: false,
            serverSide: true,
            info: false,
            pageLength: 10,
    ...
    const colleaguesTable = initializeDataTable();
    setupSearchListener(colleaguesTable);
